...
export const createValue = (model: Model, json: string, type: 'any' | 'con' | 'vec' | 'val', constOrJson?: boolean): ITimestampStruct => {
  ...
  const valueId = type === 'any'
    ? (constOrJson ? builder.constOrJson(value) : builder.json(value))
    : type === 'con'
